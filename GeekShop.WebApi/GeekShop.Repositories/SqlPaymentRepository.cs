    ...
    public class SqlPaymentRepository : IPaymentRepository
    {
        ...
        public async Task Update(Payment payment, Order order)
        {
            ...
            { 
                ...
                var sqlDeleteOrderDetails = @"
                    DELETE FROM OrderDetailsForPayments
                    WHERE OrderId = @Id";

                var sqlInsertOrderDetails = @"
                    INSERT INTO OrderDetailsForPayments
                    (OrderId, ProductTitle, ProductAuthor, ProductDescription, ProductPrice, ProductQuantity, ProductCategoryName)
                    VALUES(@OrderId, @ProductTitle, @ProductAuthor, @ProductDescription, @ProductPrice, @ProductQuantity, @ProductCategoryName)";
                ...
                await connection.QueryAsync(sqlDeleteOrderDetails, parameters, transaction: transaction);
                foreach (var orderDetail in order.Details)
                {
                    ...
                    await connection.QueryAsync(sqlInsertOrderDetails, parameters, transaction: transaction);
                }
            }
            catch
            {
